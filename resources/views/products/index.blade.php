          ...
          <div class="product-content">
            ...
            <div class="img">
                <a href="{{ route('products.show', ['product' => $product->id]) }}">
                  <img src="{{ $product->image_url ?: 'https://picsum.photos/seed/'.$product->id.'/400/400' }}" alt="{{ $product->title }}" loading="lazy" data-seed="{{ $product->id }}" onerror="this.src='https://picsum.photos/seed/'+this.dataset.seed+'/400/400'">
                </a>
              </div>
...
@section('scriptsAfterJs')
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      var filtersInput = searchForm.querySelector('input[name=filters]');
      var searches = parseSearch();

      if (searchInput) {
        searchInput.value = filters.search || searches.search || '';
      }
      if (orderSelect && filters.order) {
        orderSelect.value = filters.order;
      }
      if (filtersInput && searches.filters) {
        filtersInput.value = searches.filters;
      }

      if (orderSelect) {
        orderSelect.addEventListener('change', function () {
          var current = parseSearch();
          if (current.filters && filtersInput) {
            filtersInput.value = current.filters;
          }
          searchForm.submit();
        });
      }
